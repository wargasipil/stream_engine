...
func TestHashmap(t *testing.T) {
	...
	t.Run("testing increment float", func(t *testing.T) {
		kv.Transaction(func(tx *stream_core.Transaction) error {
			newkey := tx.IncFloat64("users/ads_spents", 2000.01)
			assert.Equal(t, float64(2000.01), newkey)

			newkey = tx.IncFloat64("users/ads_spents", 1.22)
	...
	t.Run("testing increment int", func(t *testing.T) {
		...
		kv.Transaction(func(tx *stream_core.Transaction) error {
			newkey := tx.IncInt64("product/stock", int64(1))
			...
			t.Run("adding new key", func(t *testing.T) {
				newkey = tx.IncInt64("product/stock_amount", 200)
				assert.Equal(t, int64(200), newkey)
				newkey = tx.IncInt64("product/stock_amount", 100)
	...
	// 		assert.Equal(t, int64(2), kv.GetInt64("product/stock"))
	...
	// 		value := kv.GetUint64("product/all_stock_not_exist")
	...
	t.Run("testing getting snapshot", func(t *testing.T) {
		kv.Snapshot(time.Now().Add(time.Second*-10), true, func(key string, kind reflect.Kind, value any) error {
			switch key {
